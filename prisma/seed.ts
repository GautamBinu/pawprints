...
async function main() {
  ...
  await prisma.globalAlert.create({
    data: {
      content:
        "Welcome to the new PawPrints! Please review the updated community guidelines.",
      active: true,
    },
  });
  ...
  await prisma.petition.create({
    data: {
      ...
      description:
        "<p>Students need a safe and quiet place to study late at night during finals week. We propose keeping the library open 24/7.</p>",
  ...
  const tacoResponse = await prisma.response.create({
    data: {
      description:
        "<p>We hear you! The Taco Bar will return next semester with new options including vegan chorizo.</p>",
  ...
  await prisma.petition.create({
    data: {
      ...
      description:
        "<p>The taco bar at the Commons was a staple of student diet. Please bring it back!</p>",
      authorId: studentUser.id,
      status: 1, // Published
  ...
  await prisma.petition.create({
    data: {
      ...
      description:
        "<p>The internet connection in the dorms is unstable and slow, affecting our ability to do homework.</p>",
      authorId: studentUser.id,
      status: 1, // Published
      signatures: 300,
      ...
      updates: {
        create: [
          {
            description:
              "<p>We have met with ITS and they are investigating the access points in Sol Heumann Hall.</p>",
            createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000), // 5 days ago
          },
          {
            description:
              "<p>ITS has agreed to upgrade the firmware on all routers next Tuesday.</p>",
  ...
  await prisma.petition.create({
    data: {
      ...
      description:
        "<p>Mental health is important. Weekends should be strictly for rest.</p>",
      authorId: studentUser.id,
  ...
  await prisma.petition.create({
    data: {
      ...
      description:
        "<p>The university should provide free pizza every Friday.</p>",
      authorId: studentUser.id,
      status: 2, // Removed
      signatures: 5,
      expires: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
