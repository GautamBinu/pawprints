  ...
  return (
    <div className="container max-w-5xl mx-auto py-10 px-4 sm:px-6">
        ...
        <Card className="w-full md:w-80 shrink-0 border-none shadow-sm bg-muted/30">
          <CardHeader className="flex flex-col items-center text-center pb-2">
            <Avatar className="h-24 w-24 mb-4">
              <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${profile.user.name || profile.user.email}`} />
              <AvatarFallback><UserIcon className="h-10 w-10" /></AvatarFallback>
            </Avatar>
            <CardTitle className="text-xl">{profile.user.name || 'Anonymous'}</CardTitle>
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="flex justify-between text-sm pt-4 border-t">
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">{profile.createdPetitions.length}</div>
                <div className="text-xs text-muted-foreground uppercase">Created</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-foreground">{profile.signedPetitions.length}</div>
                <div className="text-xs text-muted-foreground uppercase">Signed</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <PetitionModal
        ...
        initialIsAuthor={profile?.createdPetitions.some(p => p.id === selectedPetition?.id)}
