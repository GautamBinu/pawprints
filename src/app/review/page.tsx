...
export default function ReviewPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      console.log(`Rejecting petition ${selectedPetition.id} with reason: ${rejectReason}`);
      
      await rejectPetition(selectedPetition.id);
      setPendingPetitions((prev) => prev.filter((p) => p.id !== selectedPetition.id));
      toast.success(`Petition "${selectedPetition.title}" rejected and removed.`);
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="mb-8 space-y-2">
            <h1 className="text-3xl font-bold text-[#F76902]">Review Pending Petitions</h1>
          ...
          <div className="grid gap-4 md:grid-cols-3 mb-8">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{pendingPetitions.length}</div>
              </CardContent>
            </Card>
          </div>
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to reject "{selectedPetition?.title}"? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
